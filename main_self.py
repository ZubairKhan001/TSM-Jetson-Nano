...
def get_executor(use_gpu=True):
    torch_module = MobileNetV2(n_class=6)
    #aa="ckpt.best.pth.tar"
    #weights_list = aa.split(',')
    #for this_weights in zip(weights_list)
    #if not os.path.exists("ckpt.best.pth.tar"):  # checkpoint not downloaded
    ...
    replace_dict = {
                    'features.3.conv.0.net.weight': 'features.3.conv.0.weight',
                    'features.5.conv.0.net.weight': 'features.5.conv.0.weight',
                    'features.6.conv.0.net.weight': 'features.6.conv.0.weight',
                    'features.8.conv.0.net.weight': 'features.8.conv.0.weight',
                    'features.9.conv.0.net.weight': 'features.9.conv.0.weight',
                    'features.10.conv.0.net.weight': 'features.10.conv.0.weight',
                    'features.12.conv.0.net.weight': 'features.12.conv.0.weight',
                    'features.13.conv.0.net.weight': 'features.13.conv.0.weight',
                    'features.15.conv.0.net.weight': 'features.15.conv.0.weight',
                    'features.16.conv.0.net.weight': 'features.16.conv.0.weight',
    ...
    for k, v in replace_dict.items():
        ...
        if k in base_dict:
            ...
            #new_base_dict[k] = base_dict[k].split('.')[1]
    #print(new_base_dict)
...
catigories=["Normal", "Fighting","Shooting", "Mobbing", "Falling", "Robbery" ]
...
def main():
    print("Open camera...")
    cap = cv2.VideoCapture("atm_14.mp4")
    ...
    i_frame = -1
    out = cv2.VideoWriter('mobilenet_demo.mp4', cv2.VideoWriter_fourcc('M','J','P','G'), 30.0, (640,480+(480 // 10)))

    #out = cv2.VideoWriter('mobilenet_demo.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 30.0, (640,480))
    print("Ready!")
    while True:
        i_frame += 1
        ...
        cv2.putText(label, 'Prediction: ' + catigories[idx_],
                    (0, int(height / 16)),
